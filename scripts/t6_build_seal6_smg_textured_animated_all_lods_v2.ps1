param(
    ...
    [string]$OutDir = "retail_output\seal6_smg_textured_animated_all_lods_v2",
...
$ActualSha = (Get-FileHash -Algorithm SHA256 -LiteralPath $BaseIpak).Hash.ToLowerInvariant()
if ($ActualSha -ne $ExpectedSha) { throw "base.ipak SHA-256 mismatch: $ActualSha" }

if ($XAnimJson.Count -eq 0) {
    if (-not $XAnimDir) { throw "Provide -XAnimJson or -XAnimDir." }
    foreach ($n in @("pb_stand_alert","pb_stand_ads","pb_smg_sprint","pb_combatrun_forward_loop","pb_standjump_takeoff","pt_stand_shoot_auto")) {
        $XAnimJson += (Require-File (Join-Path $XAnimDir "$n.normalized.json") "normalized XAnim $n")
    }
} else {
    $resolved = @()
...
# Geometry exporter currently consumes the historical exact material-handle proof.
$ExportSurfaceProof = Require-File (Join-Path $Retail "seal6_smg_material_handle_alias_proof_v1.json") "export surface proof"
# Material binding consumes the durable explicit 42-row surface table.
...
$Materializer = Require-File (Join-Path $Tools "t6_ipak_iwi_materialize_v2.py") "portable IPAK materializer v2"
...
Run-With $TexturePython @($Materializer,"--ipak",$BaseIpak,"--targets",$ExactKeys,"--metadata",$TextureMeta,"--metadata",$Imports,"--outdir",$TextureDir,"--expect-ipak-sha256",$ExpectedSha)
...
if ($Tex.summary.materialized -ne 42 -or $Tex.summary.unresolved -ne 0) { throw "Expected 42/42 exact retail texture payloads." }
...
foreach ($e in $Expected) {
    ...
    $LodResults += [ordered]@{
        ...
        texturedGlb=$textured; texturedGlbSha256=(Get-FileHash -Algorithm SHA256 -LiteralPath $textured).Hash.ToLowerInvariant(); texturedGlbBytes=(Get-Item -LiteralPath $textured).Length
        nativeMaterialIdentityClosure=$B.summary.nativeIdentityClosure; visualPbrTextureClosure=$B.summary.visualPbrTextureClosure
    }
}

$Summary = [ordered]@{
    format="t6-seal6-smg-textured-animated-all-lods-build-v2"
    authority="retail T6 all serialized XModel LODs + retail common_mp animations + exact surface/material/image/base.ipak evidence"
    baseIpak=[ordered]@{path=$BaseIpak;sha256=$ActualSha}
    surfaceAssignments=[ordered]@{path=$BindingSurfaceProof;sha256=(Get-FileHash -Algorithm SHA256 -LiteralPath $BindingSurfaceProof).Hash.ToLowerInvariant();rows=42}
    portableTextureBackend=[ordered]@{python=$TexturePython;requirements=$Requirements;systemLzoDllRequired=$false}
    materializedTextures=$Tex.summary
    lods=$LodResults
    aggregate=[ordered]@{serializedLods=4;serializedSurfaces=42;verticesAcrossSerializedSurfaces=21188;trianglesAcrossSerializedSurfaces=21283;allLodsTextured=$true;allLodsAnimated=$true;allSurfaceMaterialsExact=$true;exactTexturePayloads=42}
    proofBoundary="All four serialized body LODs are closed for geometry, skin, exact surface materials, 42 CRC-verified retail texture payloads, and the six-animation benchmark. Full third-person behavior-family closure, MP7 first-person/world integration, FX/audio, and the final complete-player gate remain separate."
}
$Build = Join-Path $OutDir "build_summary.json"
$Summary | ConvertTo-Json -Depth 12 | Set-Content -Encoding UTF8 -LiteralPath $Build
Write-Host "Built all four retail SEAL6 body LODs: $OutDir"
Write-Host "Proof: $Build"
