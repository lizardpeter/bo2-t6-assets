...
#[derive(Default)]
struct PrelockedWriteEscalation { crossed_publication_boundary: Cell<bool> }
impl PrelockedWriteEscalation {
    fn crossed(&self) -> bool { self.crossed_publication_boundary.get() }
}
...
impl NativeGraph {
    ...
    fn execute_read(
        ...
        Plan { plan, cached, parameters, .. }: Plan,
    ) -> Result<QueryOutput, String> {
        let lock = ReadOnlyEscalation;
        let runtime = Runtime::new(
            snapshot, parameters, false, plan, false, self.import_folder.clone(),
            self.result_set_size, false, self.timeout_ms, 0, None, &lock,
        );
    ...
    fn execute_write(&self, cypher: &str) -> Result<QueryOutput, String> {
        ...
        let Plan { plan, cached, parameters, .. } = {
            let committed = host_guard.read();
            let graph_ref = committed.borrow();
            graph_ref.get_plan(cypher)?
        };
        debug_assert!(plan.iter().any(|n| matches!(
            n, IR::Commit | IR::CreateIndex { .. } | IR::DropIndex { .. }
        )));
        let private = host_guard.write()
            .ok_or_else(|| "native host: another MVCC write is in progress".to_string())?;
        let escalation = PrelockedWriteEscalation::default();
        let runtime = Runtime::new(
            Arc::clone(&private), parameters, true, plan, false, self.import_folder.clone(),
            self.result_set_size, false, self.timeout_ms, 0, None, &escalation,
        );
...
fn plan_is_write(plan: &Plan) -> bool {
    plan.plan.iter().any(|n| matches!(
        n, IR::Commit | IR::CreateIndex { .. } | IR::DropIndex { .. }
    ))
}

fn capture_output(runtime: &Runtime<'_>, result: &ResultSummary<'_>) -> QueryOutput {
    let columns = runtime.return_names.iter().map(ToString::to_string).collect();
    let mut rows = Vec::new();
    for batch in &result.result {
        for row_idx in batch.active_indices() {
            let row = runtime.return_names.iter().map(|var| {
                batch.value_at(var.id, row_idx)
                    .map_or_else(|| "Null".to_string(), |v| format!("{v:?}"))
            }).collect();
            rows.push(row);
        }
    }
    QueryOutput { columns, rows, stats: OutputStats::from(&result.stats) }
}
