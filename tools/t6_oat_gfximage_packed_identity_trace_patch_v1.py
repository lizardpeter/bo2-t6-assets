...
repl='''#ifdef FEATURE_T6\n        if (image.streamedPartCount > 0)\n        {\n            con::info("T6_GFXIMAGE_PACKED name={} nameHash={} dataHash={} streamedParts={} width={} height={} depth={}", image.name ? image.name : "<null>", image.hash, image.streamedParts[0].hash, static_cast<unsigned>(image.streamedPartCount), image.width, image.height, image.depth);\n            for (auto* ipak : IIPak::Repository)\n'''
