#!/usr/bin/env python3
"""Production T6 world export pipeline v32: exact specular consumer frontier.

v32 preserves v31 visual bytes.  When exact final-output and v30 specular anchor
sidecars exist, it traces completed specular XYZW roots upward toward o0 and
records nearest downstream mixing sites and exact external texture-resource
ancestry.  This is forensic/diagnostic and does not assign physical channel
semantics from resource proximity.
"""
from __future__ import annotations
import argparse,hashlib,json
from pathlib import Path
import t6_generated_final_output_specular_consumer_frontier_v1 as frontier
...
FORMAT='t6-oat-world-textured-export-pipeline-manifest-v32'
class OatTexturedPipelineV32Error(RuntimeError):pass
...
def _path(r,label):
 if not isinstance(r,dict):raise OatTexturedPipelineV32Error(f'missing {label} record')
 p=Path(str(r.get('path') or ''))
 if not p.is_file():raise OatTexturedPipelineV32Error(f'{label} does not exist: {p}')
 return p
def run_oat_textured_pipeline(**kw):
 r=v31.run_oat_textured_pipeline(**kw)
 if r.get('format')!=v31.FORMAT:raise OatTexturedPipelineV32Error(f"unexpected v31 base manifest {r.get('format')!r}")
 out=Path(kw['output_dir']);name=str(kw['map_name']);o=r.get('outputs',{})
 old=_path(o.get('oatPortableTexturedGlb'),'v31 portable GLB');gb=old.read_bytes();new=out/f'{name}.world_oat_portable_textured_v32.glb';new.write_bytes(gb)
 ...
 if had_gltf:
  oldg=_path(gr,'v31 portable glTF');g=oldg.read_bytes();newg=out/f'{name}.world_oat_portable_textured_v32.gltf';newg.write_bytes(g)
 ...
 if isinstance(final_rec,dict):
  if not isinstance(spec_rec,dict):raise OatTexturedPipelineV32Error('final-output DAG exists but v30 specular sidecar is absent')
  ...
  try:d1=frontier.build(fd,sp);d2=frontier.build(fd,sp)
  except Exception as e:raise OatTexturedPipelineV32Error(f'specular consumer-frontier tracing failed: {e}') from e
  b1=_jb(d1);b2=_jb(d2)
  if d1!=d2 or b1!=b2:raise OatTexturedPipelineV32Error('specular consumer frontier regeneration was not byte-identical')
  doc=d1;p=out/f'{name}.generated_final_output_specular_consumer_frontier_v1.json';p.write_bytes(b1);o['generatedFinalOutputSpecularConsumerFrontier']=_rec(p,b1)
 s=None if doc is None else doc['summary'];r.setdefault('stats',{})['generatedFinalOutputSpecularConsumerFrontier']=s
 r.setdefault('validation',{}).update({'v32SpecularConsumerFrontierGenerated':doc is not None,'v32SpecularConsumerFrontierDeterministic':None if doc is None else True,'v32SpecularConsumerMaterialCount':None if s is None else int(s['materialCount']),'v32SpecularConsumerChannelCount':None if s is None else int(s['channelCount']),'v32SpecularChannelWithResourceMixCount':None if s is None else int(s['channelWithResourceMixCount']),'v32SpecularMaterialWithAnyResourceMixCount':None if s is None else int(s['materialWithAnyResourceMixCount']),'v32VisualGlbByteIdenticalToV31':True,'v32VisualGltfByteIdenticalToV31':True if had_gltf else None})
 r.setdefault('policies',{})['v32SpecularConsumerFrontier']='trace completed specular XYZW roots upward through exact final-output dataflow, treating all four roots as one owned state; record nearest mixing operations and exact external texture ancestry, including parent texture samples that consume specular state as coordinates/LOD; do not infer physical channel meaning from proximity'
 oldm=r.pop('manifest',None)
 if isinstance(oldm,dict):
  p=Path(str(oldm.get('path') or ''))
  if p.is_file():p.unlink()
 r['format']=FORMAT;mb=_jb(r);mp=out/f'{name}.world_oat_textured_export_manifest_v32.json';mp.write_bytes(mb);r['manifest']=_rec(mp,mb);return r
