...
Optionally emits ordinary t6-xmodel-target-probe-v1 documents per source so the
same results feed directly into t6_first_person_bundle_plan_v1.py.
"""
...
from t6_nonmap_corpus_census_v1 import collect_inputs, safe
...
def per_source_probe(stream: Path, data: bytes, front: dict, rows: list[dict], digest: str) -> dict:
    required = [r for r in rows if r.get("required", True)]
    exact = [r for r in required if r["status"] == "exact_inline_xmodel"]
    return {
        ...
        "rules": {
            ...
            "generatedByCorpusProbe": True,
        },
        "summary": {
            "targets": len(rows),
            "requiredTargets": len(required),
            "exactRequired": len(exact),
            "allRequiredExactInline": len(exact) == len(required),
...
def main() -> int:
    ...
    ap.add_argument("--per-source-outdir", type=Path, help="also emit t6-xmodel-target-probe-v1 JSON per stream")
    ...
    if args.per_source_outdir:
        args.per_source_outdir.mkdir(parents=True, exist_ok=True)
    ...
    for stream in streams:
        ...
        probe_path = None
        if args.per_source_outdir:
            stem = safe(stream.stem) or "zone"
            short_hash = digest[:12]
            filename = f"{stem}_{short_hash}_xmodel_probe.json"
            if filename in used_probe_names:
                raise ValueError(f"per-source probe filename collision: {filename}")
            used_probe_names.add(filename)
            probe_path = args.per_source_outdir / filename
            probe_path.write_text(json.dumps(per_source_probe(stream, data, front, rows, digest), indent=2, sort_keys=True) + "\n", encoding="utf-8")
        source_rows.append({
            "path": str(stream),
            "bytes": len(data),
            "sha256": digest,
            "assetCount": front.get("asset_count"),
            "perSourceProbePath": str(probe_path) if probe_path else None,
            "perSourceProbeSha256": sha256(probe_path) if probe_path else None,
            "results": rows,
        })
        for row in rows:
    ...
    out = {
        ...
        "summary": {
            ...
            "perSourceProbesEmitted": sum(s["perSourceProbePath"] is not None for s in source_rows),
        },
        "targets": target_rows,
        "sources": source_rows,
    }
