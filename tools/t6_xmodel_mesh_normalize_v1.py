#!/usr/bin/env python3
"""Normalize inline T6 PC32 XModel render surfaces into lossless mesh data.

This first mesh layer intentionally fails closed on packed/reused surface data.
...
class Normalizer:
    ...
    def normalize(self):
        ...
        if not inline(namep): raise MeshError('v1 requires inline XModel name')
...
def main():
    ...
    a=ap.parse_args(); data=a.expanded.read_bytes(); d=Normalizer(data,a.asset_start).normalize(); d['expandedSha256']=hashlib.sha256(data).hexdigest()
