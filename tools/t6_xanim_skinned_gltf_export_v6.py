#!/usr/bin/env python3
"""Strict retail-delta-aware T6 skinned XModel + XAnim glTF exporter v6.

v6 supersedes v5's permissive delta endpoint extension. It keeps:
- generalized rigid + 1/2/3/4-influence skin validation;
- retail-proven non-root translation composition:
    XModel bind-local translation + XAnim translation delta;
- the separate __T6_DELTA_ROOT__ wrapper for XAnimParts.deltaPart;
- exact CUBICSPLINE encoding of T6 component-linear delta quaternion playback.

New v6 production rule:
- dynamic delta tracks must contain their native complete 0..numframes key
  domain. No first/last keys are invented, duplicated, or clamped.
- optional XAnimParts.frequency is checked against framerate/numframes.

Ordinary animated root-bone translation remains fail-closed. The retail
XAnim deltaPart proof closes the dedicated root-motion delta path, not the
separate ordinary bone-track/bind-pose composition question.
"""
...
from t6_xanim_skinned_gltf_export_v5 import _append_f32, _find_delta_channel, _validate_v5
...
def _replace_delta_channels(gltf: dict, raw: bytearray, xanim: dict) -> dict:
    ...
    if rot is None:
        ...
        sampler["input"] = _append_f32(
            gltf, raw, rot["times"], "SCALAR", "delta:rotation:retailNativeTime"
        )
        if rot["interpolation"] == "CUBICSPLINE":
            sampler["output"] = _append_f32(
                gltf, raw, rot["cubicRows"], "VEC4", "delta:rotation:retailNativeCubic"
            )
        else:
            sampler["output"] = _append_f32(
                gltf, raw, rot["values"], "VEC4", "delta:rotation:retailNativeValue"
            )
        sampler["interpolation"] = rot["interpolation"]
        rot_ch.setdefault("extras", {}).update({
            "t6RetailDeltaEvaluator": (
                "XAnim_CalcDeltaForTime"
                if rot["sourceType"] == "quat2"
                else "XAnim_CalcDelta3DForTime"
            ),
            "t6RetailInterpolation": (
                "component-linear stored int16 quaternion; normalized for glTF rotation"
            ),
            "t6GltfEncoding": rot["interpolation"],
            "t6ExactRetailComponentLinear": bool(rot["exactRetailComponentLinear"]),
            "t6NativeDynamicDomainValidated": bool(rot["nativeDomainValidated"]),
            "t6IndexWidthBytes": int(rot["indexWidthBytes"]),
            "t6ProofManifest": _PROOF_MANIFEST,
        })
        stats["rotation"] = True
        stats["rotationIndexWidthBytes"] = int(rot["indexWidthBytes"])
    ...
    if trans is None:
        ...
        sampler["input"] = _append_f32(
            gltf, raw, trans["times"], "SCALAR", "delta:translation:retailNativeTime"
        )
        sampler["output"] = _append_f32(
            gltf, raw, trans["values"], "VEC3", "delta:translation:retailNativeValue"
        )
        ...
        trans_ch.setdefault("extras", {}).update({
            ...
            "t6NativeDynamicDomainValidated": bool(trans["nativeDomainValidated"]),
            "t6IndexWidthBytes": int(trans["indexWidthBytes"]),
            "t6ProofManifest": _PROOF_MANIFEST,
        })
        stats["translation"] = True
        stats["translationIndexWidthBytes"] = int(trans["indexWidthBytes"])

    anim.setdefault("extras", {}).setdefault("T6", {}).update({
        "durationSeconds": duration,
        "deltaNativeDynamicDomainRequired": True,
        "deltaEndpointPolicy": "use stored native keys only; never fabricate 0/end keys",
    })
    return stats
...
def export(mesh_doc: dict, skeleton_doc: dict, xanim_doc: dict | None = None, lod: int = 0) -> dict:
    ...
    if not uri.startswith(_BUFFER_URI_PREFIX):
        raise ExportError("v6 requires embedded base64 buffer")
    ...
    if animated:
        for channel in gltf["animations"][0]["channels"]:
            ...
            # trans_values remains the separate ordinary-root safeguard: it
            # rejects parentIndex=None even though deltaPart root motion is now proven.
            ...
            sampler["output"] = _append_f32(
                gltf, raw, values, "VEC3", f"{name}:translation:bindPlusDelta"
            )
    ...
    gltf["asset"]["generator"] = "bo2-t6-assets t6_xanim_skinned_gltf_export_v6.py"
    extras = gltf.setdefault("extras", {}).setdefault("T6", {})
    extras.update({
        "skinValidation": skin,
        "translationCompositionCorrectedChannels": corrected,
        "rootTranslationPolicy": (
            "reject ordinary animated root-bone translation until separately source-closed"
        ),
        "deltaPartPolicy": (
            "retail-byte-proven deltaPart root wrapper; dynamic tracks require exact native 0..numframes domain"
        ),
