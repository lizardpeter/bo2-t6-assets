#!/usr/bin/env python3
"""Run inside Blender: structural tests for exact-local T6 lprobe nodes v2."""
from __future__ import annotations

import json
import bpy
import t6_blender_lprobe_nodes_v2 as backend
...
def main():
    ...
    uv_nodes = [n for n in opaque.node_tree.nodes if n.bl_idname == "ShaderNodeUVMap"]
    assert len(uv_nodes) == 3, len(uv_nodes)
    assert all(tuple(n.location) != (0.0, 0.0) for n in uv_nodes)
    ...
    report = {
        "format": "t6-blender-lprobe-nodes-test-v2",
        ...
        "validation": {
            ...
            "uvNodeSocketBugFixed": True,
            "allRoleCopiesNonColor": True,
        },
    }
    print("T6_BLENDER_LPROBE_V2_TEST=" + json.dumps(report, sort_keys=True))
    bpy.ops.wm.save_as_mainfile(filepath="/tmp/T6_BLENDER_LPROBE_NODES_V2_TEST.blend")
    with open("/tmp/T6_BLENDER_LPROBE_NODES_V2_TEST.json", "w", encoding="utf-8") as handle:
