...
CENSUS_FORMAT='t6-nuketown-production-texture-dependency-census-v2'; IDENTITY_FORMAT='t6-nuketown-identitynormalmap-block5-proof-v1'; LIGHTMAP_FORMAT='t6-nuketown-lightmap-inline-payload-proof-v1'; IDENTITY='$identitynormalmap'; SOURCE_SHA='7e791fb90a085f3bff9e0df895e5a232fa91cc43bcd0027225fe811f7891d505'
def raw(p): b=p.read_bytes(); return json.loads(b),b
def sha(b): return hashlib.sha256(b).hexdigest()
def build(census,identity,lightmaps):
 ...
 if len(missing)!=375 or IDENTITY not in missing or not lmnames<=set(missing): raise ValueError('missing identity set drift')
 ...
 if len(exact81)!=75 or ({IDENTITY}|lmnames)&exact81: raise ValueError('exact81 coverage drift')
 providers=set(exact81)|{IDENTITY}|lmnames; required=set(exact81)|set(missing); covered=required&providers; unresolved=required-providers; material={x['image'] for x in census['material']}
 ...
 return {'format':'t6-nuketown-production-texture-payload-coverage-v1','map':census.get('map'),'summary':{'productionUniqueImageCount':len(required),'exact81IpakDdsCoveredCount':len(exact81),'retailInlineCoveredCount':len(additional),'exactPayloadCoveredCount':len(covered),'exactPayloadMissingCount':len(unresolved),'exactPayloadCoverageRatio':len(covered)/len(required),'materialUniqueImageCount':len(material),'materialExactPayloadCoveredCount':len(material&covered),'materialExactPayloadMissingCount':len(material-covered),'materialExactPayloadCoverageRatio':len(material&covered)/len(material),'lightmapExactPayloadCoveredCount':len(lmnames),'reflectionExactPayloadCoveredCount':0},'additionalExactProviders':additional,'unresolvedProductionImages':sorted(unresolved),'proofBoundary':'The 450-identity production denominator is unchanged. Coverage means exact retail payload authority, not visual substitution. The 75 IPAK/IWI27 DDS identities remain separately attributed; $identitynormalmap and the two secondary lightmaps are added only from independent inline retail byte proofs. Reflection-probe payloads remain unresolved. No missing identity is inferred from names, dimensions, semantics, adjacency, or expected defaults.'}
def main():
 ap=argparse.ArgumentParser();ap.add_argument('--census',type=Path,required=True);ap.add_argument('--identity-proof',type=Path,required=True);ap.add_argument('--lightmap-proof',type=Path,required=True);ap.add_argument('--out',type=Path,required=True);a=ap.parse_args();c,cb=raw(a.census);i,ib=raw(a.identity_proof);l,lb=raw(a.lightmap_proof);o=build(c,i,l);o['inputs']={'census':{'sha256':sha(cb)},'identityProof':{'sha256':sha(ib)},'lightmapProof':{'sha256':sha(lb)}};payload=(json.dumps(o,indent=2,sort_keys=True)+'\n').encode();a.out.parent.mkdir(parents=True,exist_ok=True);a.out.write_bytes(payload);print(json.dumps({**o['summary'],'sha256':sha(payload)},indent=2,sort_keys=True))
