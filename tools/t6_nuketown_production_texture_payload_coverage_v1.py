...
PIXEL='8080ff80'
IDENTITY_PROOF_SHA256='4e6e05c9c68e1b393a39524b6977bca56c6dc5edfd6e094fcd512b4ebddc7a7e'
...
def build(census, proof):
 ...
 if len(missing)!=375: raise ValueError('missing identity list drift')
 ...
 required=set(exact81)|set(missing); covered=required & set(providers); unresolved=required-set(providers)
 ...
 return {
  ...
  'summary':{
   ...
   'materialExactPayloadMissingCount':len(material-covered),
...
def main():
 ...
 c,cb=raw(a.census);p,pb=raw(a.identity_proof)
 if sha(pb) != IDENTITY_PROOF_SHA256: raise ValueError(f'identity proof SHA drift: {sha(pb)}')
 o=build(c,p);o['inputs']={'census':{'sha256':sha(cb)},'identityProof':{'sha256':sha(pb)}}
