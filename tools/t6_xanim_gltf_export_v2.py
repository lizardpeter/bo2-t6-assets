#!/usr/bin/env python3
"""Export normalized T6 XAnim tracks as self-contained glTF 2.0.

Supports either:
- skeleton-bound export via --skeleton, or
- pure-delta XAnim export with no skeleton when the animation has zero bone tracks.
...
def export_gltf(skeleton_doc: dict | None, xanim_doc: dict) -> dict:
    augment_normalized_xanim(xanim_doc)
    tracks = xanim_doc.get("boneTracks", xanim_doc.get("tracks", []))
    if skeleton_doc is None:
        if tracks:
            raise ExportError("animation has bone tracks; --skeleton is required")
        bones = []
    else:
    ...
    t6_extras = {
        "skeletonIdentity": (skeleton_doc.get("identity") if skeleton_doc else None),
        "skeletonSource": (skeleton_doc.get("source") if skeleton_doc else None),
        ...
        "losslessSidecars": (["t6-xmodel-skeleton-normalized-v2", "t6-xanim-normalized-v1"] if skeleton_doc else ["t6-xanim-normalized-v1"]),
        "deltaOnly": skeleton_doc is None,
    }
    gltf = {
        "asset": {"version": "2.0", "generator": "bo2-t6-assets t6_xanim_gltf_export_v2.py"},
        "scene": 0,
...
def main() -> int:
    ...
    ap.add_argument("--skeleton", type=Path, help="t6-xmodel-skeleton-normalized-v2 JSON; omit only for zero-bone-track pure-delta animations")
    args = ap.parse_args()
    skeleton = json.loads(args.skeleton.read_text(encoding="utf-8")) if args.skeleton else None
