...
def fetch_bytes(url: str, retries: int = 5, timeout: int = 60) -> bytes:
    ...
    for attempt in range(retries):
        ...
        except Exception as exc:  # preserve final network diagnostic
            last = exc
...
def census(download_exact_size: bool = True) -> dict:
        ...
        for obj in exact_size_objects:
            ...
            downloaded.append({
                **obj,
                ...
                "mz": raw[:2] == b"MZ",
                "exactExpectedRetail": actual_sha256 == EXPECTED_RETAIL_SHA256,
            })
    ...
    return {
        ...
        "proofBoundary": (
            "This proves only what is present in the revision set returned by the current source-derived "
            "Plutonium archive endpoint. A same-name client is not retail-equivalent unless exact byte length "
            "and SHA-256 match the pinned retail identity. Archive absence is not a global absence claim."
        ),
    }
