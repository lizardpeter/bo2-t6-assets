...
def techs(d,blocks,before): return world.scan_techsets(d,blocks,before=before)
def tech_q(raw,base,blocks,assets): return world.technique_asset_index(raw,base,blocks,assets)
...
def bind_map(name,path):
 ...
 for _ in range(cfg[4]):
  ...
  assert b['q0']<=q<=b['q1']; t=byq[q]; tech=t['name']; fmt=int(t['worldVertFormat']); family=classify(r['name'],tech)
