...
The resolver never invents packed-name identities or bone aliases. A named XAnim
can be classified as an exact skeleton subset or as a retail-compatible superset
that contains animation-only controls. Unresolved animation names remain
explicit blockers rather than being guessed.
"""
...
FORMAT = "t6-playeranim-retail-resolution-v1"
...
def track_names(row: dict) -> list[str]:
    ...
    if isinstance(tracks, list):
        return [t["name"] for t in tracks if isinstance(t, dict) and isinstance(t.get("name"), str)]
    walk = row.get("walk")
...
def build_inventory(path: Path) -> dict[str, dict]:
    ...
    for row in rows:
        ...
        by_name[name] = row
    if duplicates:
        raise ValueError(f"ambiguous duplicate XAnim records: {sorted(duplicates)[:20]}")
    return by_name
...
def parse_playeranim(path: Path) -> list[dict]:
    ...
    for line_no, line in enumerate(text.splitlines(), 1):
        ...
        if match:
            rows.append({"line": line_no, "part": match.group(1), "animation": match.group(2), "source": line.rstrip()})
    return rows
...
def main() -> int:
    ...
    for name in sorted(grouped):
        ...
        if row is None:
            unresolved.append({"name": name, "references": grouped[name]})
            continue
