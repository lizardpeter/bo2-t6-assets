...
Unlike v1, this guard distinguishes *data loss* from intentional scene
reorganization. Previously proven scene roots may move to another glTF scene
(e.g. animation-support XModels moved out of the canonical map scene) as long
as they remain preserved as scene roots somewhere in the document.
...
def inspect(path: Path):
    ...
    base_names = {
        str(mats[i].get("name") or "") for i in used
        if (mats[i].get("pbrMetallicRoughness") or {}).get("baseColorTexture") is not None
    }
    normal_names = {
        str(mats[i].get("name") or "") for i in used
        if mats[i].get("normalTexture") is not None
    }
...
def main():
    ...
    ap.add_argument("--canonical-scene-name")
    ap.add_argument("--forbid-canonical-substring", action="append", default=[])
    a = ap.parse_args()
    ...
    # v1 scene roots are still mandatory, but may migrate between scenes.
    ...
    if a.canonical_scene_name and c["activeSceneName"] != a.canonical_scene_name:
        failures.append({"kind": "scene-policy", "field": "activeSceneName", "required": a.canonical_scene_name, "actual": c["activeSceneName"]})
    for needle in a.forbid_canonical_substring:
        bad = sorted(n for n in c["activeSceneRootNames"] if needle.lower() in n.lower())
        if bad:
            failures.append({"kind": "scene-policy", "field": "forbiddenCanonicalRootSubstring", "substring": needle, "count": len(bad), "names": bad})

    report = {
        "format": "t6-glb-nonregression-report-v2",
        "baseline": str(a.baseline),
        "candidate": {k: (sorted(v) if isinstance(v, set) else v) for k, v in c.items() if not k.endswith("Names")},
        "pass": not failures,
        "failureCount": len(failures),
        "failures": failures,
        "policy": "Previously proven data may move between scenes, but may not disappear. Coverage floors, material bindings and structural ceilings remain mandatory. Canonical-scene visibility policy can be enforced independently.",
    }
    if a.report:
