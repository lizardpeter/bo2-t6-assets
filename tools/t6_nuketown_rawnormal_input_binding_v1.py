...
For the exact 20-pixel-shader raw-normal lit population this adapter follows:
  ...
  destination -> exact `material.<property>` -> native OAT Material texture.
...
Constant-buffer leaves are likewise named by RDEF. They are joined to a native
Material constant only when an exact Technique material argument and exact
Material constant name establish that ownership; otherwise they remain
engine/runtime leaves.
"""
...
def one_exact(rows:list[dict],name:str,label:str):
 hits=[x for x in rows if isinstance(x,dict) and str(x.get('name') or '')==name]
 if len(hits)!=1:raise BindError(f'{label}: exact {name!r} hits {hits}')
 return hits[0]
...
def build(special_path:Path,raw_path:Path,material_root:Path,rdef_tool:Path):
 ...
 # Per exact shader, recover one byte-identical RDEF and exact Technique arguments.
 shader_info={}
 for rp in raw.get('programs',[]):
  ...
  old=shader_info.get(hh)
  cur={'rdef':rd,'arguments':amap,'shaderPath':str(path),'shaderSha256':hh}
  if old:
 ...
 material_sample_bindings=0;engine_sample_bindings=0;material_constant_bindings=0;engine_constant_leaves=set();used_images=set();used_sampler_states=set();used_engine_resources=set();out=[]
 for hh in sorted(shader_info):
  ...
  for site in rr.get('sampleSites',[]):
   ...
   tex=rdef._one_resource(rd,rdef.INPUT_TEXTURE,reg,'t')
   sam=rdef._one_resource(rd,rdef.INPUT_SAMPLER,sreg,'s')
   tname=str(tex['name']);sname=str(sam['name'])
   # Exact Technique destinations are shader identifiers. No string rewriting.
   arg=amap.get(tname)
   if arg is None and sname!=tname:arg=amap.get(sname)
   if arg is not None:
    prop=str(arg.get('materialProperty') or '')
    mt=one_exact(textures,prop,f'{hh}/t{reg}')
    ...
    material_sample_bindings+=1;used_images.add(image);used_sampler_states.add(json.dumps(state,sort_keys=True,separators=(',',':')))
    binding={'ownership':'material','techniqueArgument':arg,'materialTexture':{'name':mt.get('name'),'semantic':mt.get('semantic'),'image':image,'samplerState':state}}
   else:
    engine_sample_bindings+=1;used_engine_resources.add((reg,tname,sreg,sname));binding={'ownership':'engine_or_code','techniqueArgument':None,'materialTexture':None}
   sample_rows.append({'sampleSite':site,'rdefTexture':tex,'rdefSampler':sam,**binding})

  # Name all actually-read cbuffer leaves from the full DAG.
  crows=[]
  ...
  for sym in symbols:
   c=rdef._resolve_cb_symbol(rd,sym);v=str(c['variable']);arg=amap.get(v)
   owned=None
   if arg is not None:
    prop=str(arg.get('materialProperty') or '')
    hits=[x for x in constants if isinstance(x,dict) and str(x.get('name') or '')==prop]
    if len(hits)==1:
     owned={'ownership':'material','techniqueArgument':arg,'materialConstant':hits[0]};material_constant_bindings+=1
    elif len(hits)>1:raise BindError(f'{hh}/{sym}: duplicate Material constant {prop!r}')
   if owned is None:
    owned={'ownership':'engine_or_code','techniqueArgument':arg,'materialConstant':None};engine_constant_leaves.add((c['constantBuffer'],v,sym))
 ...
 return {
  ...
  'summary':{
   ...
   'uniqueMaterialImageCount':len(used_images),'uniqueMaterialSamplerStateCount':len(used_sampler_states),
  ...
  'proofBoundary':'Every used raw-normal sample register is reflected from exact DXBC RDEF. A sample is material-owned only when the exact RDEF shader identifier matches an exact native Technique material-argument destination and that argument names exactly one native OAT Material texture; otherwise it remains engine/code-owned. Constant leaves use the same exact RDEF/Technique/Material rule. No suffix stripping, register semantics, family names, or shader filenames assign ownership.'
 }
