...
def scan(fd,W,T,world):
    ...
    for mname,mp in sorted(mat_ptrs.items()):
        ...
        for slot in range(tc):
            ...
            calc=rhash(iname)
            if ih!=calc:raise RuntimeError(f'{iname}: GfxImage.hash {ih:08x} != R_HashString {calc:08x}')
            part=None
            if part_count:
            ...
            key=(iname,ih,w,h,d,streaming,part_count,part['hash'] if part else None)
            prev=images.get(iname)
            image_rec={k:rec[k] for k in ('image','imageHash','width','height','depth','streaming','streamedPartCountRaw')}
    ...
    doc={
        'format':'t6-nuketown-live-world-material-texture-identities-v1',
        'map':'mp_nuketown_2020','materialCount':len(mats),'textureEntryCount':texture_entries,
        'uniqueImageCount':len(images),'semanticEntryCounts':{str(k):v for k,v in sorted(semantic_counts.items())},
        ...
        'proofBoundary':'Every record is read from the frozen retail GfxWorld surface Material pointer, its live MaterialTextureDef array and direct live GfxImage pointer. GfxImage.hash is independently required to equal R_HashString(image name). Streamed payload identity is the live streamedParts[0].hash, retained both full-width and CRC29/IPAK form.',
    }
    return doc
...
def main():
    ...
    diag={'format':'t6-oat-external-gfxworld-texture-scan-diagnostics-v1','pid':p.pid,'command':cmd};fd=None;frozen=False
    try:
        for _ in range(400):
        ...
        diag['summary']={k:doc[k] for k in ('materialCount','textureEntryCount','uniqueImageCount','semanticEntryCounts','uniqueImageStreamingCounts','uniqueImageStreamedPartCounts')}
