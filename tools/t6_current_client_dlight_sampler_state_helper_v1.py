#!/usr/bin/env python3
"""Freeze the exact current-client helper that supplies slot-16 sampler state.

Target 0x009DBC20 is the only unresolved return-value producer feeding the
remaining non-immediate dlightAttenuationSampler state write at 0x009C2B34.
"""
...
FORMAT="t6-current-client-dlight-sampler-state-helper-v1"
ANCHOR=0x009DBC20
SEARCH=0x3000
CTX=24
...
def body(raw,s,anchor):
    ...
    out=ins[p+1:q]; req(out and out[0].address==anchor,f"function start drift: {out[0].address if out else None}")
    return out
...
def main():
    ...
    ib,secs=pe(raw); s=secfor(secs,ANCHOR); ins=body(raw,s,ANCHOR); cc=callers(raw,secs,ANCHOR)
    returns=[rr(i) for i in ins if i.mnemonic.startswith("ret")]
    doc={"format":FORMAT,"authority":"SHA-classified exact current-client bounded helper and rel32 caller denominator",
      "client":{"revision":a.revision,"bytes":len(raw),"sha256":dg,"imageBaseHex":f"0x{ib:08x}"},
      "function":{"startVa":f"0x{ins[0].address:08x}","endVaExclusive":f"0x{ins[-1].address+ins[-1].size:08x}",
                  "section":s["name"],"instructionCount":len(ins),"instructions":[rr(i) for i in ins]},
      "callers":cc,
      "summary":{"instructionCount":len(ins),"directCallerCount":len(cc),"returnInstructionCount":len(returns)},
      "proofBoundary":"Exact helper bytes and direct rel32 callers only. The helper is selected solely because its EAX return feeds the slot-16 sampler-state write. Semantic sampler interpretation and resource-image ownership remain separate gates."}
