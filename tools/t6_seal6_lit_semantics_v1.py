...
def cornea_sharp_window(power_term: float, sharpness: float) -> float:
    ...
    lower = 0.72 - 0.18 * a
    upper = 0.72 + 0.18 * a
    return smooth01((power_term - lower) * (1.0 / (upper - lower)))
...
def _validate(ir: dict[str, Any], abi: dict[str, Any], glsl_dir: Path) -> dict[str, dict[str, Any]]:
    if ir.get("format") != IR_FORMAT or ir.get("sourceArtifact", {}).get("id") != SOURCE_NATIVE_ARTIFACT:
        raise ValueError("IR format/source proof drift")
    ir_rows = {row["stem"]: row for row in ir.get("programs", [])}
    if set(ir_rows) != {row[0] for row in SHADERS.values()}:
        raise ValueError(f"unexpected IR program set: {sorted(ir_rows)}")
    for key, (stem, dxbc_sha, glsl_sha) in SHADERS.items():
        row = ir_rows[stem]
        if (row["dxbc"]["sha256"], row["glsl"]["sha256"]) != (dxbc_sha, glsl_sha):
            raise ValueError(f"IR identity drift for {stem}")
        path = glsl_dir / f"{stem}.glsl"
        if not path.is_file() or _sha(path.read_bytes()) != glsl_sha:
            raise ValueError(f"translated GLSL missing/SHA-drifted for {stem}")
        text = path.read_text(encoding="utf-8")
        for fragment in REQUIRED.get(key, []):
            if fragment not in text:
                raise ValueError(f"{stem}: missing translated operation {fragment!r}")
    ...
    cornea_resources = {b["argument"] for b in families["mc_sw4_3d_char_eye_cornea_2eww29wu"]["pixelArgumentBindings"]}
...
def _skin_shared() -> dict[str, Any]:
    return {
        ...
        "directSpecular": {
            "NH": "saturate(dot(Nspec,H))", "HL": "saturate(dot(H,L))", "NL": "saturate(dot(Nspec,L))", "P": "exp2(13.0*SpecularAndGloss.a)",
            ...
            "lobe": "shape*NL/denominator", "characterFactor": "1.0-saturate(scriptVector0.x)", "C": "characterFactor*SpecularAndGloss.rgb",
            "base": "C*C+(1.0-C*C)*exp2(-10.0*HL)", "rgb": "base*lobe*Specular_Amount*(gridLightingCoordsAndVis.w*sunDiffuse.rgb)"
        },
        "reflection": {
            "R": "V-2.0*Nspec*dot(Nspec,V)", "g": "exp2(-9.27999973297119140625*saturate(dot(Nspec,-V)))", "lod": "4.0-4.0*SpecularAndGloss.a",
            "probeRgb": "sampleCubeLod(reflectionProbeSampler,R,lod).rgb/(sample.a+9.9999999747524270787835121154785e-7)",
            "p0": "1.0416667461395263671875*SpecularAndGloss.a", "p1": "0.4749999940395355224609375*SpecularAndGloss.a",
            "p2": "0.01822919957339763641357421875*SpecularAndGloss.a-0.015625", "p3": "0.25*SpecularAndGloss.a+0.75",
            "q": "p0*min(g,p1)+p2", "F": "saturate((C*C)*(p3-q)+q)", "rgb": "probeRgb*F*(vertexColor.a*Reflection_Amount)*reflectionLightingRatio"
        },
        "diffuse": {
            "encoded": "Diffuse_Map.rgb*(vertexColor.rgb*vertexColor.rgb)", "linearLike": "encoded*encoded",
            ...
            "lighting": "32.0*vertexColor.a*modelLighting+saturate(dot(L,Ndiff))*(gridLightingCoordsAndVis.w*sunDiffuse.rgb)", "rgb": "linearLike*lighting",
        ...
        "final": {"preHero": "directSpecular.rgb+reflection.rgb+diffuse.rgb", "heroRgb": "vec3(dot(vec4(preHero,1),heroLightingR),dot(vec4(preHero,1),heroLightingG),dot(vec4(preHero,1),heroLightingB))", "foggedRgb": "fogColor+fogVisibility*(heroRgb-fogColor)", "outRgb": "sqrt(foggedRgb*hdrControl0.x)", "outAlpha": "1.0"},
    }


def build(ir: dict[str, Any], abi: dict[str, Any], glsl_dir: Path) -> dict[str, Any]:
    families = _validate(ir, abi, glsl_dir)
    ...
    return {
        "format": FORMAT,
        "proofBoundary": "Direct finite-value normalization of five exact SHA-pinned SEAL6 ordinary-lit DXBC programs after validated DXBC->SPIR-V translation, SPIRV-Cross pretty-printing, and RDEF argument closure. Family selection is exact VS+PS SHA only. No generic PBR substitution, filename role inference, or retail-client duplicate-precedence claim.",
        "proof": {"shaderIrRun": RUN, "shaderIrArtifactId": ARTIFACT, "shaderIrArtifactDigest": ARTIFACT_DIGEST, "sourceNativeShaderRun": SOURCE_NATIVE_RUN, "sourceNativeShaderArtifactId": SOURCE_NATIVE_ARTIFACT, "glslSha256": {key: row[2] for key, row in SHADERS.items()}},
        ...
        "families": [
            {"id": "seal6-char-skin-hero-lit-v1", "techniqueSet": hero["techniqueSet"], "vertexDxbcSha256": hero["vertexShaderSha256"], "pixelDxbcSha256": hero["pixelShaderSha256"], "materialArguments": hero["pixelMaterialArguments"], "normal": {"baseXY": "Normal_Map.rg*4.01574802398681640625-2.01574802398681640625", "detailUV": "UV0*Normal_Detail_Scale", "combinedXY": "baseXY+(Normal_Detail_Map(detailUV).rg*4.01574802398681640625-2.01574802398681640625)", "Nspec": "normalize(Nbase+combinedXY.x*T+combinedXY.y*B)", "Ndiff": "normalize(Nbase+(combinedXY*Diffuse_Normal_Height).x*T+(combinedXY*Diffuse_Normal_Height).y*B)"}, "inheritsExactSharedSkinSemantics": True, "activeRetailClientWholeMaterialOwnerResolved": False, "ordinaryLitExecutablePairInvariantAcrossPhysicalHeadParents": True},
            {"id": "seal6-char-skin-standard-lit-v1", "techniqueSet": standard["techniqueSet"], "vertexDxbcSha256": standard["vertexShaderSha256"], "pixelDxbcSha256": standard["pixelShaderSha256"], "materialArguments": standard["pixelMaterialArguments"], "normal": {"normalXY": "Normal_Map.rg*4.01574802398681640625-2.01574802398681640625", "Nspec": "normalize(Nbase+normalXY.x*T+normalXY.y*B)", "Ndiff": "normalize(Nbase+(normalXY*Diffuse_Normal_Height_Facing).x*T+(normalXY*Diffuse_Normal_Height_Facing).y*B)"}, "inheritsExactSharedSkinSemantics": True},
            {"id": "seal6-char-eye-cornea-lit-v1", "techniqueSet": cornea["techniqueSet"], "vertexDxbcSha256": cornea["vertexShaderSha256"], "pixelDxbcSha256": cornea["pixelShaderSha256"], "materialArguments": cornea["pixelMaterialArguments"], "vertexMaterialArguments": cornea["vertexMaterialArguments"],
             "fillDirectionVertexTransport": {"F1": "-normalize(vec3(dot(Fill_Direction,inverseViewMatrixRow0.xyz),dot(Fill_Direction,inverseViewMatrixRow1.xyz),dot(Fill_Direction,inverseViewMatrixRow2.xyz)))", "F2": "-normalize(vec3(dot(Fill_Direction2,inverseViewMatrixRow0.xyz),dot(Fill_Direction2,inverseViewMatrixRow1.xyz),dot(Fill_Direction2,inverseViewMatrixRow2.xyz)))", "note": "Exact translated row-dot use of inverseViewMatrix rows 40..42."},
             ...
             "highlightHelper": {"Vcamera": "normalize(-worldPosition)", "H": "normalize(Vcamera-F)", "powerTerm": "exp2(log2(saturate(dot(Nsurface,H)))/max(Size,9.9999997473787516355514526367188e-6))", "a": "1.0-Sharpness", "lower": "0.7200000286102294921875-0.180000007152557373046875*a", "upper": "0.7200000286102294921875+0.180000007152557373046875*a", "windowT": "saturate((powerTerm-lower)*(1.0/(upper-lower)))", "window": "windowT*windowT*(3.0-2.0*windowT)", "sideT": "saturate((dot(Nsurface,-F)+0.20000000298023223876953125)*5.0)", "side": "sideT*sideT*(3.0-2.0*sideT)", "lobe": "window*side*Brightness"},
...
def main() -> int:
    ...
    parser.add_argument("--ir-manifest", type=Path, required=True)
    parser.add_argument("--abi-manifest", type=Path, required=True)
    parser.add_argument("--glsl-dir", type=Path, required=True)
    parser.add_argument("--out", type=Path, required=True)
    args = parser.parse_args()
    out = build(json.loads(args.ir_manifest.read_text()), json.loads(args.abi_manifest.read_text()), args.glsl_dir)
