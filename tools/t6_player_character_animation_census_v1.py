...
def build(profile_doc:dict,profile_path:Path,compatibility_paths:list[Path]):
    ...
    for path in compatibility_paths:
        d=load(path)
        if d.get("format")!="t6-third-person-animation-compatibility-v2": raise ValueError(f"{path}: unsupported compatibility format")
        ...
        key=(body,pid); rec={"path":str(path),"sha256":sha256(path),"allRequiredTracksCompatible":bool(d.get("allRequiredTracksCompatible")),"ownershipResolvedFromRetail":bool(d.get("ownershipResolvedFromRetail")),"resolutionFormat":d.get("resolutionFormat"),"summary":d.get("summary")}
    ...
    return {"format":FORMAT,"authority":"aggregation of retained third-person animation compatibility proofs; no missing cross-product cell is inferred","profileCensus":{"path":str(profile_path),"sha256":sha256(profile_path),"profileCount":len(profile_by_id)},"summary":{"bodyModels":len(bodies),"selectorProfiles":len(profile_by_id),"crossProductCells":len(cells),"closedCells":closed,"failedProofCells":failed,"pendingCells":len(cells)-closed-failed},"bodies":sorted(bodies,key=str.casefold),"cells":cells,"proofBoundary":"Closed means a retained compatibility-v2 artifact exists for the exact body/profile pair and reports both retail ownership resolution and all required tracks compatible. Missing artifacts remain pending even if a prior run was verbally reported successful. Source-layer member weapons are informational and are not final named-weapon assignments."}
