...
4. enumerates every inline top-level TechniqueSet fixed record structurally in
   source order and requires a 1:1 count with inline type-7 XAsset headers,
5. replays the full source-closed TechniqueSet serializer only at the exact
   XAsset starts referenced by the target Materials, including passes, shaders,
   vertex declarations and arguments,
...
def scan_inline_techsets(data:bytes,blocks:tuple[int,...])->list[dict[str,Any]]:
    """Structural census of inline TechniqueSet fixed records only.

    This intentionally does *not* parse nested Technique/pass children.  The
    census answers only the top-level loader-order question: which serialized
    fixed record corresponds to each inline type-7 XAsset.  Nested replay is
    performed later only for XAssets reached by exact Material pointers.
    """
    ...
    while True:
        ...
        refs=[int.from_bytes(data[off+8+4*i:off+12+4*i],"little") for i in range(36)]
        if not any(refs):off+=1;continue
        ok=True
        for raw in refs:
            if not raw:continue
        ...
        try:
            ...
            off+=1;continue
        out.append({"start":off,"name":name,"worldVertFormat":world,"techniqueRefCount":sum(1 for x in refs if x)})
        off+=152
    return out
...
def build(expanded:Path,report_path:Path,zone:str)->dict[str,Any]:
    ...
    if len(scanned)!=len(inline_q):raise ResolveError(f"inline TechniqueSet structural census {len(scanned)} != inline XAsset count {len(inline_q)}")
    ...
    for row in materials:
        ...
        try:
            ...
            raise ResolveError(f"TechniqueSet XAsset {q} nested replay failed at {s['start']}: {exc}") from exc
        if name!=s["name"]:raise ResolveError(f"TechniqueSet XAsset {q}: fixed-record identity {s['name']!r} != replay {name!r}")
        if int(node.get("worldVertFormat",-1))!=int(s["worldVertFormat"]):raise ResolveError(f"TechniqueSet XAsset {q}: worldVertFormat changed during replay")
    ...
    return {"format":FORMAT,"zone":zone,"source":{"expandedBytes":len(data),"expandedSha256":digest},"xassetVirtualLayout":layout,"xassetPointerFieldVirtualBase":base,"pointerIntersectionCandidateBases":candidates,"inlineTechniqueSetCensus":scanned,"bindings":bindings,"techniqueSets":[defs[q] for q in sorted(defs)],"summary":{"materials":len(bindings),"uniqueTechniqueSets":len(defs),"inlineTechniqueSetXAssets":len(inline_q),"allBindingsExact":True,"xassetBaseDerivedFromLoaderOrder":True,"techniqueSetSourceOrderCensusExact":True},"proofBoundary":"Material::techniqueSet is resolved through its packed VIRTUAL offset onto the exact inline type-7 XAsset pointer-field lattice. The XAsset-array VIRTUAL base is independently derived by replaying T6 XAssetList loader allocation order; the 24-byte fixed XAssetList is loaded before XFILE_BLOCK_VIRTUAL and therefore contributes zero block bytes. TechniqueSet starts are assigned by a fail-closed 1:1 structural fixed-record/type-7 source-order census. Full nested Technique/pass/shader replay is then required only for the exact TechniqueSets reached by target Materials."}
