...
def main():
    ...
    if c.get("format")!="t6-oat-parent-dependency-conflict-census-v1": raise SystemExit("unexpected conflict format")
    if c.get("authoritativeWinnerSelection") is not False: raise SystemExit("source conflict census unexpectedly selects winners")
    if o.get("client",{}).get("sha256")!=CLIENT_SHA: raise SystemExit("unexpected current-client identity")
    scalar={int(x["classifier"]):int(x["ordinal"]) for x in o.get("classifierMap",o.get("constructedRowMap",[]))}
    # Some projector versions expose the complete map under evaluationSummary.
    for x in o.get("evaluationSummary",[]):
    ...
    for x in c["conflicts"]:
        ...
        for owner in owners:
            ...
            if cl not in scalar: raise SystemExit(f"no scalar for owner {owner} classifier 0x{cl:x}")
            vals.append({"owner":owner,"classifier":cl,"classifierHex":f"0x{cl:08x}","scalar":scalar[cl]})
        if vals[0]["scalar"]<vals[1]["scalar"]: rel="owner0_lt_owner1"
        elif vals[0]["scalar"]>vals[1]["scalar"]: rel="owner0_gt_owner1"
        else: rel="equal"
        relations[rel]+=1
        rows.append({
            "techniqueSet":x.get("techniqueSet"),"technique":x.get("technique"),
            "parentOwners":owners,"ownerScalars":vals,"scalarRelation":rel,
            "winner":None,"winnerAuthority":False,
        })
    out={
      "format":"t6-current-client-five-root-scalar-relation-v1",
      "authority":"exact five-root retail FastFile ownership + SHA-classified current Plutonium client scalar mapping; no historical-retail winner authority",
      "clientSha256":CLIENT_SHA,"sources":{"conflicts":cs,"ordinals":os},
      "summary":{"conflictCount":len(rows),"relations":dict(relations),"winnerCount":0},
      "conflicts":rows,
      "proofBoundary":"The scalar values are byte-proven in the classified current client and owner identities are exact in the five-root retail FastFile census. This join proves only pairwise scalar ordering. It intentionally does not infer which linked record is lookup-visible or survives duplicate insertion, and therefore selects no current-client or historical-retail Technique winner."
    }
    if len(rows)!=58 or relations!={"owner0_lt_owner1":58}: raise SystemExit(f"unexpected relation census {len(rows)} {relations}")
    payload=(json.dumps(out,indent=2,sort_keys=True)+"\n").encode(); a.out.parent.mkdir(parents=True,exist_ok=True);a.out.write_bytes(payload)
    print(json.dumps({"bytes":len(payload),"sha256":hashlib.sha256(payload).hexdigest(),**out["summary"]},indent=2,sort_keys=True))
