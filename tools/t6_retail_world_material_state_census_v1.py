#!/usr/bin/env python3
"""Retail-byte census of T6 GfxWorld Material render-state serialization."""
from __future__ import annotations
import argparse, collections, hashlib, json, math, struct
from pathlib import Path
F=0xffffffff; I=0xfffffffe; MASK=(1<<29)-1
MAPS={
 'mp_nuketown_2020':('7e791fb90a085f3bff9e0df895e5a232fa91cc43bcd0027225fe811f7891d505',154653476,84465666,327,84618654),
 'mp_raid':('d3874c5981a01d75b72c9584e0e7d8af6132f28135f1a1ec6c4db2b9ac5037f8',156092603,86602090,352,86768156),
 'mp_hijacked':('8bae6fdafd459f3fa6794a093a895c3c12ad3ca3f37ab83170a966884d54d42b',141043798,76243098,236,76363007),
}
...
def one(name,path,cfg):
 exsha,exbytes,start,count,last=cfg; d=path.read_bytes(); h=sha(d)
 ...
 if rows[-1]['end']!=last: raise ValueError(f'{name}: final end')
...
def build(root:Path):
 maps=[one(n,root/f'{n}.expanded.bin',c) for n,c in MAPS.items()]; allstates=sorted({v for m in maps for v in m['_states']})
 for m in maps: del m['_states']
 summary={k:sum(m['stats'][k] for m in maps) for k in ('materialCount','textureCount','inlineImageCount','inlineLoadDefCount','inlineResourceBytes','constantCount','stateCount','activeRouteSlotCount','referencedStateCount','runtimeD3DPointerSlotCount')}; summary.update(retainedMapCount=3,runtimeD3DNonzeroPointerCount=0,routingFailureCount=0,enumFailureCount=0,constantValidationFailureCount=0,uniqueRawStateCount=len(allstates),uniqueDecodedStateCount=len(allstates),retainedCorpusExecutionClean=True)
 return {'format':'t6-retail-world-material-state-census-v1','producer':'tools/t6_retail_world_material_state_census_v1.py','reference':{'openAssetToolsRepository':'Laupetin/OpenAssetTools','openAssetToolsCommit':'7d027e8f89118196713e955b0e11f8404149c54d','materialZoneCode':'src/ZoneCode/Game/T6/XAssets/Material.txt','gfxImageZoneCode':'src/ZoneCode/Game/T6/XAssets/GfxImage.txt','materialFixedBytes':104,'materialTextureDefBytes':16,'materialConstantDefBytes':32,'gfxStateBitsFixedBytes':20,'gfxStateBitsLoadBitsBytes':8,'gfxImageFixedBytes':80,'gfxImageLoadDefHeaderBytes':12},'maps':maps,'summary':summary,'uniqueStateSignatureSetSha256':dig([f'{v:016x}' for v in allstates]),'proofBoundary':'Direct retail-byte execution proof for the 915 GfxWorld surface materials in the three retained MP worlds mp_nuketown_2020, mp_raid, and mp_hijacked. It walks nested FOLLOW GfxImage records, validates Material stateBitsEntry/constants/sampler-bearing texture records and full 20-byte GfxStateBits records, and checks the 8-byte loadBits vocabulary against exact T6 enum ranges. Archive roots hash each complete serialized Material byte range. It does not claim closure for the 28 missing hash-pinned MP maps, non-world/layered component materials, numeric backend polygon bias, technique selection, or visual playback parity.'}
def main():
 ap=argparse.ArgumentParser(); ap.add_argument('--root',type=Path,default=Path('/mnt/data/t6_xanim_corpus/mp')); ap.add_argument('--out',type=Path,required=True); a=ap.parse_args(); doc=build(a.root); a.out.parent.mkdir(parents=True,exist_ok=True); a.out.write_text(json.dumps(doc,indent=2,sort_keys=True)+'\n'); print(json.dumps(doc['summary'],indent=2,sort_keys=True))
