#!/usr/bin/env python3
"""One-command audited T6 GfxWorld + native OAT materials/DDS -> textured GLB.

Direct retail-oriented material path:
  raw world sidecars -> audited base world pipeline
  OAT materials/**/*.json + exact world material catalog -> semantic manifest
  OAT DDS image directory -> deterministic PNG staging
  normalized world + semantic manifest + staged images -> textured glTF/GLB

No Frost/Blender material CSV is required for ordinary exact OAT material
identities. Generated/compound material identities still fail closed unless an
exact OAT material JSON exists; their compositor syntax is a separate proof
boundary.
"""
...
def run_oat_textured_pipeline(
    *,
    ...
    material_path = output_dir / f"{stem}.oat_material_texture_manifest.json"
    material_path.write_bytes(material_bytes)
    material_sha = _sha256(material_bytes)

    texture_dir = output_dir / f"{stem}.dds_textures"
    ...
    stage_path = output_dir / f"{stem}.dds_texture_stage_manifest.json"
    ...
    if raw2 != raw1 or glb2 != glb1:
        raise OatTexturedPipelineError(
            "OAT textured GLB regeneration was not byte-identical"
        )

    textured_glb_path = output_dir / f"{stem}.world_oat_textured.glb"
    ...
    if write_gltf:
        ...
        if gltf_payload1 != gltf_payload2:
            raise OatTexturedPipelineError(
                "OAT textured glTF regeneration was not byte-identical"
            )
        gltf_deterministic = True
        gltf_path = output_dir / f"{stem}.world_oat_textured.gltf"
    ...
    manifest = {
        "format": "t6-oat-world-textured-export-pipeline-manifest-v1",
        "map": map_name,
        "inputs": {
        ...
        "validation": {
            ...
            "missingOatMaterialCount": material_manifest["stats"][
        ...
        "stats": {
            ...
            "oatMaterials": material_manifest["stats"],
            "ddsStage": stage_manifest["stats"],
            "texturedGltf": textured_stats,
        },
        "policies": {
            "materialJoin": (
                "exact GfxWorld catalog material name == relative OAT material JSON path"
            ),
            "textureSemantics": (
            ...
            "compoundMaterials": (
                "not synthesized from generated names; unresolved identities remain an "
                "explicit blocker"
            ),
            "geometryReference": (
    ...
    manifest_path = output_dir / f"{stem}.world_oat_textured_export_manifest.json"
