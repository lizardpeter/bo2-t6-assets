...
This is the generic non-map counterpart to the retail-proven Nuketown IPAK/IWI
pipeline. A texture is promoted only when all identity and payload checks agree:
...
import argparse, ctypes, ctypes.util, hashlib, io, json, struct, zlib
...
def load_targets(path:Path)->list[dict]:
    ...
    rows=doc if isinstance(doc,list) else doc.get('exactBaseIpakKeys') or doc.get('exactStreamKeyImages')
    ...
    for r in rows:
        ...
        nh=parse_u32(r.get('nameHash',r.get('nameHashHex')))
        dh=parse_u32(sp.get('dataHash',sp.get('dataHashHex',r.get('dataHash',r.get('dataHashHex'))))) & 0x1fffffff
        key=(nh,dh)
        if key in seen:continue
        seen.add(key)
        out.append({'image':name,'nameHash':nh,'dataHash':dh,'target':r})
    if not out:raise TextureError('no exact texture targets')
    return out

def add_meta(index:dict[str,list[dict]], name:str, meta:dict):
    ...
    for path in paths:
            ...
            if isinstance(rows,list):
                for r in rows:
                    ...
                    if isinstance(name,str):
                        sp=r.get('streamedPart0') or {}
                        add_meta(candidates,name,{
                            'sourceManifest':str(path),'nameHash':r.get('nameHash',r.get('nameHashHex')),
                            'dataHash':sp.get('dataHash',sp.get('dataHashHex',r.get('dataHash',r.get('dataHashHex')))),
                            'width':r.get('width'),'height':r.get('height'),'depth':r.get('depth'),
                            'uses':r.get('uses') or [],
                        })
        ...
        if isinstance(rows,list):
            for r in rows:
                ...
                name=im.get('name')
                if isinstance(name,str):
                    sp=im.get('streamedPart0') or {}
                    add_meta(candidates,name,{
                        'sourceManifest':str(path),'nameHash':im.get('nameHash',im.get('nameHashHex')),
                        'dataHash':sp.get('dataHash',sp.get('dataHashHex')),
                        'width':im.get('width'),'height':im.get('height'),'depth':im.get('depth'),
                        'uses':r.get('uses') or [],
                    })
    out={}
...
def parse_ipak(path:Path)->dict:
    ...
    for i in range(idx['itemCount']):
        ...
        row={'index':i,'dataHashRaw':dhraw,'dataHash':dh,'nameHash':nh,'relativeOffset':rel,'entrySpan':span,'absoluteOffset':data['offset']+rel}
        rows.append(row);by.setdefault((nh,dh),[]).append(row)
...
def decode_bc1(src:bytes,w:int,h:int)->bytes:
    out=bytearray(w*h*4);p=0
    for by in range((h+3)//4):
        for bx in range((w+3)//4):
            if p+8>len(src):raise TextureError('BC1 top mip truncated')
            c0,c1,bits=struct.unpack_from('<HHI',src,p);p+=8;r0,g0,b0=rgb565(c0);r1,g1,b1=rgb565(c1)
            cols=[(r0,g0,b0,255),(r1,g1,b1,255)]
            if c0>c1:cols += [((2*r0+r1)//3,(2*g0+g1)//3,(2*b0+b1)//3,255),((r0+2*r1)//3,(g0+2*g1)//3,(b0+2*b1)//3,255)]
            else:cols += [((r0+r1)//2,(g0+g1)//2,(b0+b1)//2,255),(0,0,0,0)]
            for py in range(4):
                y=by*4+py
                if y>=h:continue
                for px in range(4):
    ...
    return bytes(out)
def decode_bc3(src:bytes,w:int,h:int)->bytes:
    out=bytearray(w*h*4);p=0
    for by in range((h+3)//4):
        for bx in range((w+3)//4):
            ...
            c0,c1,cbits=struct.unpack_from('<HHI',src,p+8);p+=16;r0,g0,b0=rgb565(c0);r1,g1,b1=rgb565(c1)
            ...
            for py in range(4):
                ...
                for px in range(4):
                    x=bx*4+px
                    if x>=w:continue
                    i=py*4+px;ci=(cbits>>(2*i))&3;ai=(abits>>(3*i))&7;o=(y*w+x)*4;out[o:o+4]=bytes((*cols[ci],al[ai]))
    return bytes(out)
def decode_bc4_block(block:bytes)->list[int]:
    ...
    if a0>a1:vals=[a0,a1]+[((6-i)*a0+(i+1)*a1)//7 for i in range(6)]
    else:vals=[a0,a1]+[((4-i)*a0+(i+1)*a1)//5 for i in range(4)]+[0,255]
    return [vals[(bits>>(3*i))&7] for i in range(16)]
def decode_bc5_normal(src:bytes,w:int,h:int)->bytes:
    import math
    out=bytearray(w*h*4);p=0
    for by in range((h+3)//4):
        for bx in range((w+3)//4):
            ...
            for py in range(4):
                ...
                for px in range(4):
                    x=bx*4+px
                    if x>=w:continue
                    i=py*4+px;xn=xs[i]/127.5-1.0;yn=ys[i]/127.5-1.0;zn=math.sqrt(max(0.0,1.0-xn*xn-yn*yn));z=int(round((zn*0.5+0.5)*255));o=(y*w+x)*4;out[o:o+4]=bytes((xs[i],ys[i],max(0,min(255,z)),255))
    return bytes(out)
def top_png(iwi:bytes,is_normal:bool)->tuple[bytes,dict]:
    ...
    top=((w+3)//4)*((h+3)//4)*block;end=len(iwi);start=end-top
    ...
    im=Image.frombytes('RGBA',(w,h),rgba);buf=io.BytesIO();im.save(buf,format='PNG',optimize=False)
    return buf.getvalue(),{**m,'topMipOffset':start,'topMipBytes':top,'normalSemantic':bool(is_normal)}
...
def main()->int:
    ...
    for t in targets:
        ...
        if len(matches)!=1:
            unresolved.append({**t,'reason':'exact-key-not-found' if not matches else 'ambiguous-exact-key','matchCount':len(matches)});continue
        ...
        try:
            ...
            normal=any((isinstance(x,str) and ('normalMap' in x or x.endswith(':normal'))) or (isinstance(x,dict) and (x.get('semanticName')=='normalMap' or x.get('semanticRaw')==5)) for x in uses)
            png,png_meta=top_png(payload,normal)
        except Exception as exc:
            unresolved.append({**t,'reason':'payload-validation-failed','error':str(exc)});continue
        stem=f"{safe_name(t['image'])}__nh_{t['nameHash']:08x}__dh_{t['dataHash']:08x}";iwi_name=stem+'.iwi';png_name=stem+'.png';(a.outdir/iwi_name).write_bytes(payload);(a.outdir/png_name).write_bytes(png)
        row={**t,'index':matches[0]['index'],'entrySpan':matches[0]['entrySpan'],'iwiFile':iwi_name,'pngFile':png_name,'iwiSha256':sha256_bytes(payload),'pngSha256':sha256_bytes(png),'retainedMetadata':meta,'iwi':png_meta,'crc29Validated':True,'exactKeyValidated':True}
        rows.append(row);print(f"{t['image']}: {iwi_meta['width']}x{iwi_meta['height']} fmt=0x{iwi_meta['format']:02x} -> {png_name}")
    doc={'format':'t6-ipak-iwi-materialization-v1','authority':'exact retail stream keys + T6 IPAK raw/LZO reconstruction + CRC29 + IWI27 + retained dimension validation','sourceIpak':{'path':str(a.ipak),'bytes':a.ipak.stat().st_size,'sha256':actual_sha,'versionHex':f"0x{ipak['version']:x}"},'targetManifest':{'path':str(a.targets),'sha256':sha256_file(a.targets)},'metadataManifests':[{'path':str(p),'sha256':sha256_file(p)} for p in a.metadata],'summary':{'requested':len(targets),'materialized':len(rows),'unresolved':len(unresolved),'iwiFiles':len(rows),'pngFiles':len(rows)},'textures':rows,'unresolved':unresolved,'proofBoundary':'IWI is the authoritative reconstructed retail payload. PNG is a top-mip derivative. No filename fallback, dataHash-only fallback, or unsupported-format substitution is permitted.'}
