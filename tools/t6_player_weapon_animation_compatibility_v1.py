...
The selector profile is deliberately weapon-metadata shaped (weaponclass,
playerAnimType, next variants, and forbidden runtime condition keys). It is not
a weapon name. This keeps characters reusable across every weapon whose retail
WeaponDef resolves to the same selector metadata.
"""
from __future__ import annotations
import argparse,hashlib,json
from pathlib import Path

FORMAT='t6-third-person-animation-compatibility-v1'
...
def main():
 ...
 if resolution.get('format')!='t6-playeranim-retail-resolution-v1':raise ValueError('unsupported compatibility resolution')
 ...
 for n in names:
  ...
  r=resolved[n];bindings.append({'name':n,'bindingClass':r.get('bindingClass'),'trackCount':r.get('trackCount'),'matchedTrackCount':r.get('matchedTrackCount'),'extraAnimationTrackCount':r.get('extraAnimationTrackCount'),'rawStructOffset':r.get('rawStructOffset'),'numFrames':r.get('numFrames'),'frameRate':r.get('frameRate')})
 source_hashes=[]
